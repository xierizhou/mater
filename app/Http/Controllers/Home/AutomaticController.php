...
class AutomaticController extends Controller
{
    ...
    public function register(Request $request){
        ...
        if($token && $user = User::where('token',$token)->first()){
            ...
            $sign = '';
            $browser_map = $this->_getclientbrowser_for_userauth($_SERVER['HTTP_USER_AGENT']);
        ...
        if ($res) {
            Cookie::queue('LT', $token, 60*24*30);
    ...
    private function makeUsername(){
        $time = ((time()+rand(1000,9999))*rand(1,9))/rand(1,9);
        $time = round($time);
        if($user = User::where('username',$time)->first()){
            $this->makeUsername();
        }else{
            return $time;
        }
     ...
     */
    private function _getclientbrowser_for_userauth($_useragent){
        if(!empty($_useragent)){
    ...
    private function _getclientoperation_for_userauth($_useragent){
        if(!empty($_useragent)){
            $ua = strtolower($_useragent);
            $operation_map = array();
    ...
    private function _getIpAddress($ip){
