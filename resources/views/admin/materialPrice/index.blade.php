                            ...
                            <form class="layui-form layui-col-space5">
                                ...
                                <div class="layui-input-inline layui-show-xs-block">
                                    <select name="material_id">
                                        <option value="">素材网站</option>
                                        @foreach($material as $item)
                                        <option {{ request('material_id') == $item->id ? "selected":"" }} value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <table class="layui-table layui-form">
                                ...
                                @foreach($data as $kk=>$item)
                                    <tr>
                                        ...
                                        <td>{{ $item->price }}</td>
                                        <td>{{ $item->type }}</td>
                                        <td>{{ $item->status?"正常":"下架" }}</td>
                                        <td>{{ $item->created_at }}</td>
                                        <td class="td-manage">
                                            {{--<a title="下载" onclick="xadmin.open('编辑','{{ url('admin/channel/'.$item->id.'/edit') }}',800,600)" href="javascript:;"><i class="icon iconfont">&#xe714;</i></a>--}}
                                            <a title="编辑" onclick="xadmin.open('编辑','{{ url('admin/material_price/'.$item->id.'/edit') }}',800,600)" href="javascript:;"><i class="layui-icon">&#xe63c;</i></a>
    ...
    <script>layui.use(['laydate', 'form'],
        ...
        function member_del(obj, id) {
            ...
            function(index) {
                //发异步删除数据
                $.ajax({
                    url:"{{ url('admin/material_price') }}/"+id,
                    method:'post',
